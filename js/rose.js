...
// 缩短 Math 对象中方法的引用
with(m = Math) {
    C = cos;  // 将 Math.cos 缩写为 C
    S = sin;  // 将 Math.sin 缩写为 S
    P = pow;  // 将 Math.pow 缩写为 P
    R = random;  // 将 Math.random 缩写为 R
}

c.width = c.height = f = 600; // 设置 canvas 的宽度和高度为 600
h = -250; // 设置变量 h 的值为 -250
function p(a, b, c) {
    if (c > 60)
        return [
